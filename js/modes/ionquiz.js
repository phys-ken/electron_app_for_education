...
const renderIonQuiz = () => {
  ...
  const renderStartScreen = () => {
    ...
    startScreen.innerHTML = `
      <h2 class="text-xl font-bold mb-6">イオンクイズ</h2>
      <p class="mb-4">イオン式に合わせて正しい電子配置を作ってください！</p>
      <div class="mb-6">
  ...
  const renderQuestion = () => {
    ...
    const chargeText = currentIon.charge > 0 ? 
      `<sup>${currentIon.charge}+</sup>` : 
      `<sup>${Math.abs(currentIon.charge)}-</sup>`;
    
    ionInfo.innerHTML = `
      <div class="text-xl font-bold">${currentIon.element}${chargeText}</div>
      <div class="text-sm text-gray-600">${currentIon.name}</div>
    `;
    ...
    const drawAtom = () => {
      ...
      ctx.fillText(`+${currentIon.z}`, centerX, centerY);
    ...
    checkBtn.addEventListener('click', () => {
      ...
      // 正解の電子数
      const correctElectronCount = currentIon.z - currentIon.charge;
      const correctConfig = configForZ(correctElectronCount);
